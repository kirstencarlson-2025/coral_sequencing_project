...
# including quality filtering, removal of symbiont reads, alignment to a de novo reference genome, and indel variant ca$
...
# unique_reads -> merge_unique_reads -> filter_tags -> tab_to_fasta -> cluster_tags -> kraken2_filter -> copy_denovo_ref -> construct_denovo_ref -> index_denovo_ref -> cleanup_denovo_intermediate
#####################################################
...
rule kraken2_filter:
    ...
    shell:
        """
        kraken2 --db {params.kraken_db} --output /dev/null --report {merge_dir}/kraken_report.txt --unclassified-out {output} {input}
        """
