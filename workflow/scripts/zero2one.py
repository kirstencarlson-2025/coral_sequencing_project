# ----------------------------------------------------- #
# Modified from zero2one.py in DiscoSnp++:
# https://github.com/GATB/DiscoSnp/blob/master/scripts/zero2one.py
# Original authors: GATB

# Modified by: Kirsten Carlson
# Last edit: 2/24/2026
# - Added support for seperate output file (-o)
# - Removed in-place editing for Snakemake compatibility
# ----------------------------------------------------- #
...
def zero2one(input_vcf_file_name, output_vcf_file_name):
    with open(input_vcf_file_name) as vcf_file, open(output_vcf_file_name, "w") as outfile:
...
if __name__ == "__main__":
    ...
    parser.add_argument("-o", help="output one-based VCF file", dest="output_vcf_file", type=str, required=True)
    args = parser.parse_args()
    zero2one(args.input_vcf_file, args.output_vcf_file)
    print(f"Transformed zero-based {args.input_vcf_file} to one-based {args.output_vcf_file}")
